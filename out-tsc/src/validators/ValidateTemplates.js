...
import { identity, newIssueEvent, } from '../foundation.js';
...
async function validateCoOperStructure(oper) {
    ...
    if (!type)
        return [];
    const datype = oper.closest('DataTypeTemplates')?.querySelector(`DAType[id="${type}"]`) ??
        null;
...
async function validateCoSBOwStructure(sbow) {
    ...
    if (!type)
        return [];
    const datype = sbow.closest('DataTypeTemplates')?.querySelector(`DAType[id="${type}"]`) ??
        null;
    const nsd81 = await iec6185081;
    const errors = [];
...
export async function validateControlCDC(dotype) {
    ...
    else if (ctlModel === 'direct-with-normal-security' ||
...
async function getMandatorySubDAs(datype) {
    const parentDAs = Array.from(datype
        .closest('DataTypeTemplates')
        .querySelectorAll(`DOType > DA[type="${datype.getAttribute('id')}"]`)) ?? [];
...
export default class ValidateTemplates extends LitElement {
    async validate(identity, statusNumber) {
        const promises = [];
