...
export function newValidateEvent(identity = '', eventInitDict) {
...
function lNodeSelector(tagName, identity) {
    if (identity.endsWith(')')) {
        ...
        return tags[tagName].parents
            .map(parentTag => `${selector(parentTag, parentIdentity)}>${tagName}[iedName="None"][lnClass="${lnClass}"][lnType="${lnType}"]`)
            .join(',');
    }
