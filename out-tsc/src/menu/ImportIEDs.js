...
function importIedsAction(importDoc, doc) {
    return (_, wizard) => {
        ...
        const promises = selectedItems
            ...
            .filter(ied => ied)
            .map(ied => importIED(ied, doc, wizard));
...
function addLNodeType(ied, lNodeType, doc) {
    ...
    if (existLNodeType) {
        ...
        ied
            .querySelectorAll(`AccessPoint > Server > LDevice > LN0[lnType="${idOld}"], AccessPoint > Server > LDevice > LN[lnType="${idOld}"]`)
...
export async function importIED(ied, doc, dispatchObject) {
    if (ied.getAttribute('name') === 'TEMPLATE')
        ied.setAttribute('name', 'TEMPLATE_IED' +
            (Array.from(doc.querySelectorAll('IED')).filter(ied => ied.getAttribute('name')?.includes('TEMPLATE')).length +
                1));
