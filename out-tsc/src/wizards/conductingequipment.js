...
export function createConductingEquipmentWizard(parent) {
    const reservedNames = Array.from(parent.querySelectorAll('ConductingEquipment'))
        .filter(isPublic)
...
export function editConductingEquipmentWizard(element) {
    const reservedNames = Array.from(element.parentNode.querySelectorAll('ConductingEquipment'))
        .filter(isPublic)
        .map(condEq => condEq.getAttribute('name') ?? '')
