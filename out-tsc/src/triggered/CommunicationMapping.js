...
import { createElement, findControlBlocks, getReference, identity, newWizardEvent, pathParts, selector, } from '../foundation.js';
    ...
    return [
        ...
        >
          <filtered-list
            ...
            >${sourceIEDs
                    .filter(sourceIED => sourceIED.getAttribute('name') !==
                    sinkIED.getAttribute('name'))
                    .flatMap(sourceIED => {
                    return Array.from(sourceIED.getElementsByTagName('ReportControl')).map(cb => {
                        return {
                            identity: identity(cb),
                            numberClientLNs: Array.from(cb.getElementsByTagName('ClientLN')).filter(clientLN => clientLN.getAttribute('iedName') ===
                                sinkIED.getAttribute('name')).length,
                        };
                    });
                })
...
function getSelectedSourceIedElements(evt, doc) {
    const selectedItems = ((evt.target.parentElement?.querySelector('#sourcelist') ??
        []).selected) ?? [];
...
function sinkPrimary(sink) {
    const ln = (sink.getAttribute('prefix') ?? '') +
...
//# sourceMappingURL=CommunicationMapping.js.map