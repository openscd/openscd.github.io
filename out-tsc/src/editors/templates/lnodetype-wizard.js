...
function dOWizard(options) {
    ...
    const [title, action, type, deleteButton, name, desc, accessControl, transientList,] = DO
        ? [
...
function getDescendantClasses(nsd74, base) {
    if (base === '')
        return [];
    const descendants = getDescendantClasses(nsd74, nsd74
...
function startLNodeTypeCreate(parent, templates, nsd74) {
    return (inputs, wizard) => {
        ...
        if (existId)
            return [];
        const desc = getValue(inputs.find(i => i.label === 'desc'));
        const lnClass = inputs.find(i => i.label === 'lnClass')?.selected
            ?.value;
        ...
        const newLNodeType = templateLNodeType && autoimport
