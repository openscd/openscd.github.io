...
import { createElement, getReference, getValue, identity, isPublic, newActionEvent, newWizardEvent, patterns, selector, } from '../../foundation.js';
import { addReferencedDataTypes, allDataTypeSelector, buildListFromStringArray, predefinedBasicTypeEnum, updateIDNamingAction, valKindEnum, } from './foundation.js';
...
function bDAWizard(options) {
    ...
    const [title, action, type, deleteButton, name, desc, bTypeList, sAddr, valKindList, valImportList,] = bda
        ? [
        ...
        ></wizard-textfield>`,
          ...
          @selected=${(e) => {
                    ...
                    Array.from(typeUI.children).forEach(child => {
                        ...
                        child.style.display = !child.classList.contains(bType)
                            ? 'none'
                            : '';
                        child.selected =
                            bTypeOriginal === bType
