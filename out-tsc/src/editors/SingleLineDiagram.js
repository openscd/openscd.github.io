...
import { getAbsolutePosition, drawRouteBetweenElements, DEFAULT_ELEMENT_SIZE, createTerminalElement, createBusBarElement, createVoltageLevelElement, createBayElement, createConductingEquipmentElement, createConnectivityNodeElement, getElementDimensions, getAbsolutePositionConnectivityNode, getBusBarLength, createPowerTransformerElement, } from './singlelinediagram/sld-drawing.js';
import { isBusBar, getConnectedTerminals, getPathNameAttribute, getNameAttribute, } from './singlelinediagram/foundation.js';
...
export default class SingleLineDiagramPlugin extends LitElement {
    ...
    drawConductingEquipments() {
        Array.from(this.doc.querySelectorAll('ConductingEquipment'))
            .filter(child => Array.from(child.querySelectorAll('Terminal')).filter(terminal => terminal.getAttribute('cNodeName') !== 'grounded').length !== 0)
            .forEach(equipment => {
        ...
        this.bays.forEach(bay => {
            Array.from(bay.querySelectorAll('ConnectivityNode'))
                ...
                Array.from(this.doc.querySelectorAll('ConductingEquipment,PowerTransformer'))
                    .filter(cEquipment => cEquipment.querySelector(`Terminal[connectivityNode="${cNode.getAttribute('pathName')}"]`))
                    .forEach(cEquipment => {
    ...
    drawBusBarConnections() {
        this.busBars.forEach(busBar => {
            ...
            Array.from(this.doc.querySelectorAll('ConductingEquipment'))
                .filter(cEquipment => cEquipment.querySelector(`Terminal[connectivityNode="${pathName}"]`))
                .forEach(cEquipment => {
                const cEquipmentAbsolutePosition = getAbsolutePosition(cEquipment);
                let sideToDrawTerminalOn;
     ...
     */
    addElementToGroup(elementToAdd, identity) {
        this.svg
