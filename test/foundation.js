import fc, {array, hexaString, integer, tuple} from "../_snowpack/pkg/fast-check.js";
import {patterns} from "../src/foundation.js";
export function regexString(re, minLength = 0, maxLength) {
  return fc.string({minLength, maxLength: maxLength ?? 2 * minLength + 10}).filter((char) => re.test(char));
}
